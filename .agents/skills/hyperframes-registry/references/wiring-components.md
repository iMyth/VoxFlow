...
```js
// Cover screen
tl.to('.grid-cell', { scale: 1, duration: 0.6, stagger: { amount: 0.6, from: 'center' }, ease: 'power2.inOut' }, 5.0);
// Swap scenes
tl.set('#scene-a', { opacity: 0 }, 5.6);
tl.set('#scene-b', { opacity: 1 }, 5.6);
// Reveal
tl.to('.grid-cell', { scale: 0, duration: 0.6, stagger: { amount: 0.6, from: 'edges' }, ease: 'power2.inOut' }, 5.6);
```
