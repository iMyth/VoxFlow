...
export default function PlanCard({
  ...
  const allMapped = plan.suggested_characters.every((c) => characterMapping[c.name] && !creatingChars[c.name]);
    ...
    <Card className="border-blue-200 dark:border-blue-800/60 bg-blue-50/80 dark:bg-blue-950/40">
      <CardContent className="space-y-4 py-4">
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-blue-700 dark:text-blue-300">{t('editor.planTitle')}</h3>
        ...
        <div>
          ...
          <div className="space-y-1">
            {plan.chapters.map((ch, i) => (
              <div
                key={i}
                className="flex items-center gap-2 text-xs bg-card/80 dark:bg-card/50 rounded-md border border-border/30 px-2.5 py-2"
              >
                ...
                <span className="text-muted-foreground shrink-0">
                  {ch.estimated_lines} {t('editor.planLines')}
                </span>
        ...
        <div>
          ...
          <div className="space-y-2">
            {plan.suggested_characters.map((ch, i) => (
              <div
                key={i}
                className="space-y-2 bg-card/80 dark:bg-card/50 rounded-md border border-border/30 px-3 py-2.5"
              >
                <div className="flex items-center gap-2">
                  ...
                  {ch.matched_existing && (
                    <Badge
                      variant="outline"
                      className="text-xs bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800/60 shrink-0"
                    >
                      ...
                      {importGroups.length > 0 && (
                        <SelectGroup>
                          ...
                          {importGroups.map(([_projectId, projectName, chars], pi) =>
                            chars.map((c, ci) => (
                              <SelectItem key={c.id} value={`__import:${String(pi)}:${String(ci)}`}>
                                {c.name} ({projectName})
                              </SelectItem>
                            ))
                          )}
                  ...
                  <div className="ml-2 mt-2 p-3 rounded-md border border-dashed border-border/60 space-y-2.5 bg-card/40">
                    <div className="flex items-center gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          onFormChange(ch.name, 'name', e.target.value);
                        }}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          onFormChange(ch.name, 'voice', e.target.value);
                        }}
                        className="h-7 text-xs"
                      />
                    </div>
                    <div className="flex gap-4">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            onFormChange(ch.name, 'speed', parseFloat(e.target.value) || 1.0);
                          }}
                        />
                      </div>
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            onFormChange(ch.name, 'pitch', parseFloat(e.target.value) || 0.0);
                          }}
                        />
                      </div>
                    </div>
                    <div className="flex justify-end gap-2 pt-1">
                      <Button
                        ...
                        onClick={() => {
                          onCancelNewChar(ch.name);
                        }}
                      >
                        {t('character.cancel')}
                      </Button>
                      <Button size="sm" className="h-6 text-xs" onClick={() => void onNewChar(ch.name)}>
        ...
        <div>
          <p className="text-xs font-medium text-blue-600 dark:text-blue-400 mb-1">
            {t('editor.planExtraInstructions')}
          </p>
          <textarea
        ...
        <div className="flex justify-end pt-1">
          ...
          >
            {isGenerating ? <Loader2 className="h-3 w-3 mr-1 animate-spin" /> : <Check className="h-3 w-3 mr-1" />}
